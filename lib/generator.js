...
exports.copyStatic = function (folder, output, config) {
  ...
  for (let index = 0; index < blessingJSONFiles.length; index++) {
    ...
    if (path.basename(file, 'json') === 'blessings.') {
      ...
      fs.writeFileSync(path.join(output, 'all.html'), pug.renderFile(path.join(folder, 'assets', 'pug', 'all.pug'), Object.assign(config, jsonfile.readFileSync(path.join(folder, 'blessings', file)))))
    } else {
